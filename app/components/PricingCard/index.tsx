...
function PricingRow({ label, value, dataTestId }: PricingRowProps) {
  ...
  return (
    <div className={styles.row}>
      ...
      <div data-testid={dataTestId} className={styles.pricingRowValue}>
        ${prettyValue}
      </div>
    </div>
  );
...
function PricingCard() {
  ...
  const tipAmountPerPerson =
    people == 0 ? 0 : (bill * ((tip?.value ?? 0) / 100)) / people;
  const totalAmountPerPerson =
    people == 0 ? 0 : bill / people + tipAmountPerPerson;
  ...
  const handleStandardBillClick =
    (standardTip: number) => (e: React.ChangeEvent<HTMLInputElement>) => {
      setTip((_prev) =>
    ...
    <div className={styles.card}>
      <div className={styles.inputsWrapper}>
        <Input
          ...
          Icon={
            <Image src="icon-dollar.svg" alt="dollar" width={12} height={12} />
          }
        />
        ...
        <div>
          ...
          <div className={styles.tipsWrapper}>
            {STANDARD_TIPS.map((standardTip) => {
              const checked =
                tip?.type == "standard" && tip?.value == standardTip;
        ...
        <Input
          ...
          Icon={
            <Image src="icon-person.svg" alt="dollar" width={12} height={12} />
          }
        />
      </div>
      ...
      <div className={styles.resultWrapper}>
        <div className={styles.pricingRowsWrapper}>
          <PricingRow
            ...
            label="Tip Amount"
            dataTestId="tip-amount-value"
          />
          <PricingRow
            value={totalAmountPerPerson}
            label="Total"
            dataTestId="total-amount-value"
          />
        </div>
